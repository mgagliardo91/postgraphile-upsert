...
const PgMutationUpsertPlugin = (builder) => {
    builder.hook("GraphQLObjectType:fields", (fields, build, context) => {
        const { extend, gql2pg, graphql: { GraphQLObjectType, GraphQLInputObjectType, GraphQLNonNull, GraphQLString, }, inflection, newWithHooks, parseResolveInfo, pgGetGqlInputTypeByTypeIdAndModifier, pgGetGqlTypeByTypeIdAndModifier, pgIntrospectionResultsByKind, pgQueryFromResolveData: queryFromResolveData, pgSql: sql, pgViaTemporaryTable: viaTemporaryTable, pgField, pgOmit: omit, } = build;
        const { scope: { isRootMutation }, fieldWithHooks, } = context;
        if (!isRootMutation)
            return fields;
        return extend(fields, pgIntrospectionResultsByKind.class
            .filter((table) => !!table.namespace)
            .filter((table) => !!table.primaryKeyConstraint)
            .filter((table) => !omit(table, "upsert"))
            ...
            memo[fieldName] = fieldWithHooks(fieldName, (context) => {
                ...
                return {
                    ...
                    async resolve(data, { where, input }, { pgClient }, resolveInfo) {
                        ...
                        const constraintColumns = uniqueConstraints.reduce((acc, constraint) => ({
                            ...acc,
                            [constraint.name]: new Set(constraint.keyAttributeNums.map((num) => attributes.find((attr) => attr.num === num))),
                        }), {});
                        const upsertedColumns = new Set(attributes.filter((attr) => {
                        ...
                        const matchingConstraint = where
                            ? Object.entries(constraintColumns).find(([_, columns]) => [...columns].every((col) => where[inflection.camelCase(col.name)] !== undefined))
                            : Object.entries(constraintColumns).find(([_, columns]) => [...columns].every((col) => upsertedColumns.has(col.name))) ??
                                Object.entries(constraintColumns).find(([key]) => key === primaryKeyConstraint.name);
                        ...
                        attributes.forEach((attr) => {
                            ...
                            if (where &&
                                Object.prototype.hasOwnProperty.call(where, inflection.camelCase(attr.name))) {
                        ...
                        const conflictUpdateArray = sqlColumns.map((col) => sql.query `${sql.identifier(col.names[0])} = excluded.${sql.identifier(col.names[0])}`);
                        ...
                        ${sqlColumns.length
                            ...
                            on conflict on constraint ${sql.identifier(constraintName)}
