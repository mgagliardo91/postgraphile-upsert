...
const determineUpsertConstraint = ({ table, args: { input, where }, constraints, attributes, inflection }) => {
    ...
    const primaryKeyConstraint = constraints.find((con) => con.type === "p");
            ...
            return {
                ...
                async resolve(_data, { where: whereRaw, input }, { pgClient }, resolveInfo) {
                    ...
                    const matchingConstraint = determineUpsertConstraint({
                        ...
                        constraints: allUniqueConstraints.filter((con) => con.classId === table.id),
                        attributes: pgIntrospectionResultsByKind.attribute.filter((attr) => attr.classId === table.id),
                        inflection,
                    });
                    const inputData = input[inflection.tableFieldName(table)];
